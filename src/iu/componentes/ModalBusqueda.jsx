  ...
  return (
    ...
    >
      ...
      {productosEncontrados.length <= 0 ? (
        <p className="text-center " style={{fontSize:"14px"}}>Buscar Producto</p>
      ) : (
        <>
          {productosEncontrados.map((producto) => (
            <div key={producto._id} className="d-flex my-3 p-1 card-producto rounded-3">
              ...
              <section className="ps-3">
                ...
                <div>
                  <button className="rounded-5 m-2 enlace-producto" onClick={()=>{
                    handleClose()
          ...
          <Link to={`/busqueda/${busquedaUrl}`} className="text-center d-block bg-danger link-light">
            Ver más
          </Link>
        </>
      )}
